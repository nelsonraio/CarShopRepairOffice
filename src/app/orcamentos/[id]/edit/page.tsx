...
const EditBudgetPage = () => {
  ...
  const [budgetEstado, setBudgetEstado] = useState('pendente');
  ...
  const fetchBudget = async (budgetIdParam: string) => {
    try {
      const response = await fetch(`/api/orcamentos?id=${encodeURIComponent(budgetIdParam)}`);
      if (!response.ok) {
      ...
      setBudgetId(orcamento.ref_orcamento);
      setBudgetEstado(orcamento.estado || 'pendente');
      setAlternateContact({
        nome: orcamento.contacto_nome || '',
        telefone: orcamento.contacto_telefone || '',
        email: orcamento.contacto_email || ''
      });
      ...
      if (orcamento.cliente) {
        const client: Client = {
          ...
          perfil: orcamento.cliente.perfil || 'Normal'
        };
      ...
      if (orcamento.veiculo) {
        setFormData(prev => ({
          ...prev,
          matricula: orcamento.veiculo.matricula || '',
          marca: orcamento.veiculo.marca || '',
          modelo: orcamento.veiculo.modelo || '',
          ano: orcamento.veiculo.ano ? String(orcamento.veiculo.ano) : ''
        }));
        setQuilometragem(orcamento.kms || '');
        setSelectedVehicle(String(orcamento.veiculo.id));
        setIsVehicleAutoFilled(true);
      }

      const formatDateForInput = (rawDate: unknown): string => {
        if (!rawDate) return '';
        ...
        // Final fallback through Date parser
      ...
      setDataEmissao(formatDateForInput(orcamento.data_emissao) || today);

      const mappedItems: BudgetItem[] = (orcamento.itens_orcamento || []).map((item: any) => ({
        id: item.servico_id ? String(item.servico_id) : item.peca_id ? String(item.peca_id) : `custom-${item.id}`,
        name: item.descricao,
        quantity: Number(item.quantidade),
        unitPrice: Number(item.preco_unitario),
        unit: item.tipo_item === 'servico' ? 'h' : 'un',
        total: Number(item.valor_total),
        type: item.tipo_item === 'servico' ? 'service' : 'part'
      }));

      setBudgetItems(mappedItems);
      setTotal(Number(orcamento.total_geral || 0));
    } catch (error) {
  ...
  const updateBudget = async () => {
    if (budgetItems.length === 0) {
      alert('Adicione pelo menos um item ao orçamento antes de guardar.');
      return;
    }
    ...
    // Validar contacto alternativo se o telefone principal nao existir
    ...
    if (!clienteTelefone) {
      ...
      if (!nomeAlternativo || !telefoneAlternativo) {
        alert('O cliente nao tem telefone registado. Por favor, preencha o Nome e Telefone do contacto alternativo.');
        return;
      }
    }
    ...
    try {
      ...
      const budgetData = {
        ...
        preparado_por: null,
        data_emissao: dataEmissao || new Date().toISOString().split('T')[0],
        data_expiracao: null,
        estado: budgetEstado,
      ...
      const response = await fetch(`/api/orcamentos?id=${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        alert(`Orçamento ${budgetId} atualizado com sucesso!`);
        router.push('/orcamentos');
      } else {
        const error = await response.json();
        alert(`Erro ao atualizar orçamento: ${error.error || 'Erro desconhecido'}`);
      }
    } catch (error) {
      console.error('Error updating budget:', error);
      alert('Erro ao atualizar orçamento. Verifique a conexão com o servidor.');
    }
  };
        ...
        <div className="max-w-5xl mx-auto bg-gray-700 rounded-none shadow-lg border border-gray-600">
          <header className="bg-gray-900 rounded-t-none p-6 border-b border-gray-600">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-100">Editar Orçamento</h1>
          ...
          <div className="p-6">
            ...
            <div className="bg-gray-800 p-4 border border-gray-600 rounded-none mb-8">
              ...
              {selectedClient && !selectedClient.telefone?.trim() && (
                <div className="bg-yellow-900/20 border border-yellow-600 text-yellow-200 px-4 py-2 rounded mb-4">
                  <p className="text-sm">O cliente nao tem telefone registado. E obrigatorio preencher o Nome e Telefone alternativo.</p>
                </div>
              )}
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-end mt-6">
                <div className="bg-gray-800 border border-gray-600 rounded-none p-4 w-full max-w-sm">
                  <div className="flex justify-between text-lg font-semibold text-gray-200">
                    <span>Total do Orçamento:</span>
                    <span className="font-mono text-brand-yellow">€{total.toFixed(2)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-end items-center gap-3 px-6 pb-8 pt-4">
            <Link href="/orcamentos" className="px-4 py-2 bg-gray-600 text-gray-200 font-medium hover:bg-gray-500 transition-colors rounded-none flex items-center">
              <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
              </svg>
