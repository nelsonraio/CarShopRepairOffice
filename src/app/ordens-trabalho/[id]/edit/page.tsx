...
interface WorkOrder {
  ...
  cliente_nome: string;
  veiculo_info: string;
  itens_ordem_trabalho: WorkOrderItem[];
  mecanico: {
    id: string;
    nome: string;
  };
...
const EditWorkOrderPage = () => {
  ...
  const fetchWorkOrder = async (id: string) => {
    try {
      ...
      setFormData({
        matricula: data.veiculo_info?.split(' | ')[1] || '',
        marca: data.veiculo_info?.split(' | ')[0]?.split(' ')[0] || '',
        modelo: data.veiculo_info?.split(' | ')[0]?.split(' ').slice(1).join(' ') || '',
        ano: '',
        cliente: data.cliente_nome || '',
        problemDescription: data.descricao_problema || '',
        workDone: data.trabalho_realizado || '',
        recommendations: data.recomendacoes || '',
        priority: data.prioridade || 'normal',
        startDate: data.data_inicio || '',
        endDate: data.data_conclusao || ''
      });

      setSelectedMechanic(data.mecanico_id || '');
      setSelectedVehicle(data.veiculo_id || '');
      
      // Load client
      if (data.cliente_id) {
        const clientResponse = await fetch(`/api/clientes/${data.cliente_id}`);
        if (clientResponse.ok) {
          const clientData = await clientResponse.json();
          setSelectedClient(clientData);
          setClientSearch(clientData.nome || '');
          if (clientData.id) {
            fetchClientVehicles(clientData.id.toString());
          }
        }
      }
  ...
  const updateWorkOrder = async () => {
    ...
    try {
      ...
      const workOrderData = {
        id: workOrderId,
        ref_ordem_trabalho: workOrderId,
      ...
      if (response.ok) {
        ...
        alert(`Ordem de Trabalho ${workOrderId} atualizada com sucesso!\nTotal: €${total.toFixed(2)}\nItens: ${workOrderItems.length}`);
        router.push('/ordens-trabalho');
      } else {
        const error = await response.json();
        alert(`Erro ao atualizar ordem de trabalho: ${error.error || 'Erro desconhecido'}`);
      }
    } catch (error) {
      console.error('Error updating work order:', error);
      alert('Erro ao atualizar ordem de trabalho. Verifique a conexão com o servidor.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="flex h-screen bg-gray-800">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-gray-400">Carregando ordem de trabalho...</div>
        </main>
      </div>
    );
  }
      ...
      <div className="flex h-screen bg-gray-800">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="bg-red-900 border border-red-700 text-red-200 p-4 rounded-none">
            Erro ao carregar ordem de trabalho: {error}
          </div>
        </main>
      </div>
    );
        ...
        <div className="max-w-5xl mx-auto bg-gray-700 rounded-none shadow-lg border border-gray-600">
          <header className="bg-gray-900 rounded-t-none p-6 border-b border-gray-600">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-100">Editar Ordem de Trabalho</h1>
                <p className="text-sm text-gray-400 mt-1">ID: <span className="font-mono text-brand-yellow">{workOrderId}</span></p>
              </div>
